...
def auth_dialog(limit_reached=False):
    ...
    if st.session_state.auth_step == "select":
        ...
        if st.button("🇬 Continue with Google", use_container_width=True):
            if "auth" in st.secrets and ("google" in st.secrets["auth"] or "client_id" in st.secrets["auth"]):
                st.login("google")
            else:
        ...
        if st.session_state.auth_provider == "Google" and not ("auth" in st.secrets and ("google" in st.secrets["auth"] or "client_id" in st.secrets["auth"])):
            st.warning("⚠️ Google OAuth is not configured on your server/local secrets! Please add the `[auth.google]` block to your Streamlit Cloud secrets to enable real Google OAuth.")
